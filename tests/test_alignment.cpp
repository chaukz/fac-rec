...
int main()
{
    ...
    while (cap.readFrame(frame))
    {
        ...
        for (const cv::Rect &faceRect : detector.detect(frame))
        {
            ...
            dlib::extract_image_chip(dlibImg, dlib::get_face_chip_details(shape, 150, 0.25), faceChip);

            cv::Mat chipRgb = dlib::toMat(faceChip);
            cv::Mat chipBgr;
            cv::cvtColor(chipRgb, chipBgr, cv::COLOR_RGB2BGR);
            cv::imshow("aligned chip", chipBgr);
        }
        cv::imshow("landmarks test", frame);
        if (cv::waitKey(1) == 27)
            break;
    }
    return 0;
